...
?>
      ...
      <div class="col-md-2 col-md-offset-6 col-sm-2 col-sm-offset-5">
        ...
        <?php echo TITLE_TYPE . ' ' . tep_draw_pull_down_menu('type', $type_array, (tep_not_null($type) ? $type : 'daily'), 'onchange="this.form.submit();"'); ?><noscript><input type="submit" value="GO"></noscript><br />
<?php
  switch ($type) {
    case 'yearly': break;
    case 'monthly':
...
?>
          <?php echo tep_draw_hidden_field('page', $_GET['page']) . tep_draw_hidden_field('bID', $_GET['bID']) . tep_hide_session_id(); ?></form>
      </div>
      <div class="col-xs-12">
<?php
  if (function_exists('imagecreate') && ($dir_ok == true) && tep_not_null($banner_extension)) {
    ...
    switch ($type) {
      case 'yearly':
        ...
        echo tep_image(DIR_WS_IMAGES . 'graphs/banner_yearly-' . $banner_id . '.' . $banner_extension, '', '', '', '', true);
        break;
      case 'monthly':
        include(DIR_WS_INCLUDES . 'graphs/banner_monthly.php');
        echo tep_image(DIR_WS_IMAGES . 'graphs/banner_monthly-' . $banner_id . '.' . $banner_extension, '', '', '', '', true);
        break;
      default:
      case 'daily':
        include(DIR_WS_INCLUDES . 'graphs/banner_daily.php');
        echo tep_image(DIR_WS_IMAGES . 'graphs/banner_daily-' . $banner_id . '.' . $banner_extension, '', '', '', '', true);
        break;
    }
?>
      </div>
          ...
          <table class="table" style="max-width: 600px;">
            <thead>
              <tr class="dataTableHeadingRow">
               ...
               <th class="dataTableHeadingContent" style="text-align: right;"><?php echo TABLE_HEADING_VIEWS; ?></th>
               <th class="dataTableHeadingContent" style="text-align: right;"><?php echo TABLE_HEADING_CLICKS; ?></th>
             </tr>
            </thead>
            <tbody>
<?php
    for ($i=0, $n=sizeof($stats); $i<$n; $i++) {
      echo '            <tr class="dataTableRow">' . "\n" .
           ...
           '              <td class="dataTableContent" style="text-align: right;">' . number_format($stats[$i][1]) . '</td>' . "\n" .
           '              <td class="dataTableContent" style="text-align: right;">' . number_format($stats[$i][2]) . '</td>' . "\n" .
           '            </tr>' . "\n";
    }
?>
            </tbody>
...
?>
      <table class="table" width="100%"> <!-- osCommerce table foot -->
        <tr>
          <td class="smallText" style="text-align: right;"><?php echo tep_draw_bs_button(IMAGE_BACK, 'arrow-1-w', tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $_GET['page'] . '&bID=' . $_GET['bID'])); ?></td>
        </tr>
      </table>
    </div>
